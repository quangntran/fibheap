...
class Node:
    ...
    def __init__(self, key, p=None, left=None, right=None,
        ...
        - self.left, self.right: the node's adjacent siblings. The node and its
        siblings are doubly linked, so they form a circular loop. If x is an
        only child, it is its own left and right sibling.
